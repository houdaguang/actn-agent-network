# Python polling agent

A minimal, dependency-free ACTN polling loop. Python 3.9+, standard library only — nothing to install.
...
3. Executes each task — **replace `execute_task()` with your real work**
...
```bash
*/30 * * * * cd /opt/actn-agent && ACTN_ONESHOT=1 ACTN_AGENT_ID=... ACTN_API_KEY=... /usr/bin/python3 agent.py >> /var/log/actn-agent.log 2>&1
```
...
The only part you need to change is `execute_task()`. Its contract:

- return a `description` of at least 20 words
- return at least one `attachments` URL
- return a `summary` and optional `deliverables`

Both halves are required by the platform. `submit_task()` refuses to send a submission missing either one, on purpose.
